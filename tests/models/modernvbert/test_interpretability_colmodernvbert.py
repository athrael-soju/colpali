...
class TestGetNPatches:
    ...
    def test_get_n_patches_square_image(self, processor_from_pretrained: ColModernVBertProcessor):
        ...
        assert n_patches_x == n_patches_y, f"Expected equal patches for square image, got x={n_patches_x}, y={n_patches_y}"
...
class TestEndToEndInterpretability:
    ...
    def test_similarity_maps_shape(
        ...
        n_patches_x, n_patches_y = processor_from_pretrained.get_n_patches(
            (image_size_pil[1], image_size_pil[0]),  # (height, width)
            patch_size
        )
